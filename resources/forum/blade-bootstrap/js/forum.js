...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const pendingApproval = document.querySelector('#pending-approval');
    if (pendingApproval) {
        const allIds = JSON.parse(pendingApproval.dataset.allIds || '[]');
        createApp({
            setup() {
                ...
                watch(selectAll, (value) => {
                    selectedIds.value = value ? [...allIds] : [];
                });

                watch(selectedIds, (value) => {
                    selectAll.value = value.length === allIds.length && allIds.length > 0;
                    pendingApproval.selectedIds = value;
                });

                return { selectedIds, selectAll };
            }
