...
@section('content')
 ...
 <section class="content-header">
    ...
    <!-- Main content -->
    <form id="role" method="POST" action='{{ route("coordinators.updatecoordrole",$coordinatorDetails[0]->id) }}'>
    @csrf
    <section class="content">
        ...
        @endphp
                  ...
                  <div class="col-sm-2">
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="onLeave" id="onLeave" class="custom-control-input" {{$coordinatorDetails[0]->on_leave == '1' ? 'checked' : ''}} disabled />
                        <label class="custom-control-label" for="onLeave">Leave of Absense</label>
                    </div>
					</div>
					<div class="col-sm-2">
					  <div class="form-group">
		...
		<div class="card-body text-center">
			...
			<a href='{{ route("coordinators.coordroleview",$coordinatorDetails[0]->id) }}' class="btn bg-gradient-primary"><i class="fas fa-reply" ></i>&nbsp; Back</a>
...
$(document).ready(function() {
    // When the Conference dropdown changes - change Region dropdown options
   ...
   // When the Region dropdown changes - change ReportTo, DirectReports and PrimaryChapters dropdown options
    $('select[name="cord_region"]').on('change', function() {
        var regID = $(this).val();
        var confID = $('#cord_conf').val();
        var posID = $('#cord_pos').val();

        // Perform all AJAX requests only if all IDs are available
        if (confID && regID && posID) {
            // First AJAX request - Update ReportTo dropdown
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error("Error fetching reporting coordinators (cord_report_pc):", error);
                }
            });

            // Second AJAX request - Update DirectReports dropdown
            $.ajax({
                url: '{{ url("/getdirectreport") }}',
                type: "GET",
                dataType: "json",
                data: { conf_id: confID, reg_id: regID, pos_id: posID },
                success: function(data) {
                    $('select[name="SelectCoordinator"]').empty().html(data.html);
                },
                error: function(xhr, status, error) {
                    console.error("Error fetching reporting coordinators (SelectCoordinator):", error);
                }
            });

            // Third AJAX request - Update PrimaryChapters dropdown
            $.ajax({
                url: '{{ url("/getchapterprimary") }}',
                type: "GET",
                dataType: "json",
                data: { conf_id: confID, reg_id: regID, pos_id: posID },
                success: function(data) {
                    $('select[name="SelectChapter"]').empty().html(data.html);
                },
                error: function(xhr, status, error) {
                }
            });
        } else {
            // Clear all dropdowns if IDs are missing
            $('select[name="cord_report_pc"]').empty();
