                                ...
                                @foreach($categoryData['courses'] as $course)
                                    <li class="mb-2 d-flex align-items-center gap-2">
                                        ...
                                        @if(!empty($course['progress']) && $course['progress']['status'] === 'completed')
                                            <span class="badge bg-success badge-inherit-size fw-bold"><i class="bi bi-check-circle me-1"></i>Completed</span>
                                        @elseif(!empty($course['progress']) && $course['progress']['status'] === 'in_progress')
                                            <span class="badge bg-warning badge-inherit-size text-dark fw-bold"><i class="bi bi-clock-history me-1"></i>In Progress</span>
                                            <div class="progress mt-2" style="height: 8px;">
                                                <div class="progress-bar bg-primary badge-inherit-size fw-bold" style="width: {{ $course['progress']['percent'] }}%"></div>
                                            </div>
                                        ...
                                        @else
                                            <span class="badge bg-danger badge-inherit-size fw-bold"><i class="bi bi-x-circle me-1"></i>Not Started</span>
                                        @endif
                                    </li>
                                @endforeach
                            </ul>
