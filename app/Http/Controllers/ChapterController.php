...
//chapterupdate
     ...
     public function boundaryview($id)
    {
        ...
        $data=array('currentMonth'=>$currentMonth,'chapterList' => $chapterList,'regionList'=>$regionList,'primaryCoordinatorList'=>$primaryCoordinatorList,'stateArr'=>$stateArr,'countryArr'=>$countryArr,'foundedMonth'=>$foundedMonth);
    ...
    public function showIntChapter()
    {
        ...
        $data=array('countList'=>$countList,'intChapterList' => $intChapterList);
    ...
    public function showInquiriesChapter()
    {
        ...
        $data=array('inquiriesList' => $inquiriesList,'corConfId' => $corConfId,'corRegId' => $corRegId);
        ...
        public function zappedInquiriesChapter()
    {
        ...
        $data=array('inquiriesList' => $inquiriesList,'corConfId' => $corConfId);
    ...
    public function showWebsiteChapter()
    {
        ...
        if($positionId == 7){
            $websiteList = DB::table('chapters')
                            ...
                            ->orderBy('chapters.name','ASC')
                            ->get();
        }else if ($positionId == 6 || $positionId == 25){
            $websiteList = DB::table('chapters')
                            ->select('chapters.id as id','chapters.name as chapter_name','chapters.website_url as web','wb.status as web_status','chapters.website_notes as web_notes','chapters.egroup as egroup','st.state_short_name as state')
                            ->join('state as st', 'chapters.state', '=', 'st.id')
                            ->leftJoin('website_link_status as wb', 'chapters.website_link_status', '=', 'wb.id')
        ...
        $data=array('websiteList' => $websiteList);
    ...
    public function showZappedChapter()
    {
        ...
        $countList=count($chapterList);
        $data=array('countList'=>$countList,'chapterList' => $chapterList);
    ...
    public function showZappedChapterView($id)
    {
        ...
        $foundedMonth = ['1'=>'JAN','2'=>'FEB','3'=>'MAR','4'=>'APR','5'=>'MAY','6'=>'JUN','7'=>'JUL','8'=>'AUG','9'=>'SEP','10'=>'OCT','11'=>'NOV','12'=>'DEC'];  
        $currentMonth = $chapterList[0]->start_month_id;    
        $data=array('corId'=>$corId,'SECDetails'=>$SECDetails,'TRSDetails'=>$TRSDetails,'MVPDetails'=>$MVPDetails,'AVPDetails'=>$AVPDetails,'chapterList' => $chapterList,'regionList'=>$regionList,'primaryCoordinatorList'=>$primaryCoordinatorList,'stateArr'=>$stateArr,'countryArr'=>$countryArr,'foundedMonth'=>$foundedMonth,'currentMonth'=>$currentMonth);
    ...
    function addToPrezList($chapterId){
       $chapterList = DB::table('chapters')
    ...
    public function chapterDisband(Request $request){
      ...
      //  DB::beginTransaction();
        ...
        $userRelatedChpaterList = DB::table('board_details as bd')
            ...
            ->get();
        if(sizeof($userRelatedChpaterList) > 0)
        {
         ...
         public function showDonation($id)
{
        ...
        $minDateLimit = '';
        $data=array('chapterList'=>$chapterList,'maxDateLimit'=>$maxDateLimit,'minDateLimit'=>$minDateLimit);
    ...
    public function showReRegistration()
    {
        ...
        $data=array('reChapterList' => $reChapterList,'checkBoxStatus'=>$checkBoxStatus,'corConfId'=>$corConfId);
...
function reminderReRegistration(){
    $corDetails = User::find(Auth::user()->id)->CoordinatorDetails;
    $corId = $corDetails['coordinator_id'];
    $corConfId = $corDetails['conference_id'];
...
function lateReRegistration(){
     ...
     public function awardsView($id)
    {
        ...
        $data=array('currentMonth'=>$currentMonth,'chapterList' => $chapterList,'regionList'=>$regionList,'primaryCoordinatorList'=>$primaryCoordinatorList,'financial_report_array'=>$financial_report_array,'stateArr'=>$stateArr,'countryArr'=>$countryArr,'foundedMonth'=>$foundedMonth);
...
$corDetails = User::find(Auth::user()->id)->CoordinatorDetails;
    ...
    public function showBoardInfo($chapterId)
    {
            ...
            $currentMonth = $chapterDetails->start_month_id;
                ...
                $PREDetails = DB::table('incoming_board_member as bd')
                                    ...
                                    ->get();
                if(sizeof($PREDetails) == 0){
                        $PREDetails[0] = array('pre_fname' =>'','pre_lname' =>'','pre_email' =>'','pre_addr' =>'','pre_city' =>'','pre_zip' =>'','pre_phone' =>'','pre_state' =>'','ibd_id' =>''); 
                ...
                $AVPDetails = DB::table('incoming_board_member as bd')
                                    ...
                                    ->get();
                if(sizeof($AVPDetails) == 0){
                        $AVPDetails[0] = array('avp_fname' =>'','avp_lname' =>'','avp_email' =>'','avp_addr' =>'','avp_city' =>'','avp_zip' =>'','avp_phone' =>'','avp_state' =>'','ibd_id' =>''); 
                        $AVPDetails = json_decode(json_encode($AVPDetails));
                }
                ...
                $MVPDetails = DB::table('incoming_board_member as bd')
                                    ...
                                    ->get();
                if(sizeof($MVPDetails) == 0){
                    $MVPDetails[0] = array('mvp_fname' =>'','mvp_lname' =>'','mvp_email' =>'','mvp_addr' =>'','mvp_city' =>'','mvp_zip' =>'','mvp_phone' =>'','mvp_state' =>'','ibd_id' =>''); 
                    $MVPDetails = json_decode(json_encode($MVPDetails));
                }
                ...
                $TRSDetails = DB::table('incoming_board_member as bd')
                                    ...
                                    ->get();
                if(sizeof($TRSDetails) == 0){
                    $TRSDetails[0] = array('trs_fname' =>'','trs_lname' =>'','trs_email' =>'','trs_addr' =>'','trs_city' =>'','trs_zip' =>'','trs_phone' =>'','trs_state' =>'','ibd_id' =>''); 
                    $TRSDetails = json_decode(json_encode($TRSDetails));
                }
                ...
                $SECDetails = DB::table('incoming_board_member as bd')
                                    ...
                                    ->get();
                if(sizeof($SECDetails) == 0){
                    $SECDetails[0] = array('sec_fname' =>'','sec_lname' =>'','sec_email' =>'','sec_addr' =>'','sec_city' =>'','sec_zip' =>'','sec_phone' =>'','sec_state' =>'','ibd_id' =>''); 
                    $SECDetails = json_decode(json_encode($SECDetails));
                }
                
                $data = array('chapterState'=>$chapterState,'currentMonth'=>$currentMonth,'foundedMonth'=>$foundedMonth,'stateArr'=>$stateArr,'SECDetails'=>$SECDetails,'TRSDetails'=>$TRSDetails,'MVPDetails'=>$MVPDetails,'AVPDetails'=>$AVPDetails,'PREDetails'=>$PREDetails,'chapterList' => $chapterList);
    ...
    public function activateBoard($chapter_id,$lastUpdatedBy){
        ...
        $incomingBoardDetails = DB::table('incoming_board_member')
                                    ...
                                    ->get();
        $countIncomingBoardDetails = sizeof($incomingBoardDetails);                                    
        if($countIncomingBoardDetails > 0){
            ...
            try{
                ...
                $boardDetails = DB::table('board_details')
                            ...
                            ->where('chapter_id', '=', $chapter_id )
                            ->get();
                $countBoardDetails = sizeof($boardDetails);
       ...
       $filterReportingList = array_reverse($filterReportingList);
        ...
        foreach($filterReportingList as $key =>$val){
            $corList = DB::table('coordinator_details as cd')
                ...
                ->get();
            if(sizeof($corList) != 0)
                $reviewerList[] = array('cid'=>$corList[0]->cid ,'cname'=>$corList[0]->fname.' '.$corList[0]->lname.' ('.$corList[0]->pos.')');
        }
       // var_dump($reviewerList);
        //print $financial_report_array['reviewer_id'];
        $data = array('reviewerList'=>$reviewerList,'chapterid'=>$chapterId,'financial_report_array'=>$financial_report_array,'emailListCord'=>$emailListCord,'cc_string'=>$cc_string,'loggedInName'=>$loggedInName, 'balance' =>$balance,'submitted' => $submitted,'chapterDetails' => $chapterDetails);
    ...
    public function storeFinancialReport(Request $request, $chapter_id){
        ...
        if(basename($_FILES["RosterFile"]["name"] != "")){ 
            ...
            if($uploadOk){
                $file = fopen($target_file,'rb') or die("can't open file");
                ...
                $share_existed = false;
                try {$response = $dropbox->postToAPI("/sharing/create_shared_link_with_settings", ['path' => $file_name_assigned, 'settings' => ['requested_visibility' => 'public']]); }
   ...
   if(basename($_FILES["Statement2File"]["name"] != "")){ 
       ...
       try{
            ...
            else if($submitType == 'review_clear')
