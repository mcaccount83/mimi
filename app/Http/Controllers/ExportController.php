...
class ExportController extends Controller
{
    ...
    public function exportChapter($id)
    {
       ...
       if($id == '0'){
        ...
        if(sizeof($activeChapterList) > 0)
        {
            $exportChapterList = array();
    ...
    public function exportZappedChapter()
    {
      ...
      /*  $zappedChapterList = DB::table('chapters')
        ...
        if(sizeof($zappedChapterList) > 0)
        {
            $exportZapChapterList = array();
    ...
    public function exportReReg()
    {
        ...
        if(sizeof($ReRegList) >= 0)
        {
            $exportReRegList = array();
            ...
            $columns = array('Conference', 'Region', 'State', 'Name', 'Primary Coordinator','Status', 'Month Due','Year Due', 'Re-Reg Notes', 'Dues Last Paid', 'Members paid for');
...
/**
    ...
    public function exportEINStatus()
    {
        ...
        if(sizeof($activeChapterList) > 0)
        {
            ...
            $columns = array('Conference', 'Region', 'State', 'Name', 'EIN', 'EIN Leter', 'EIN Path', 'Start Month', 'Start Year', 'First Name', 'Last Name', 'Address', 'City', 'State', 'Zip', 'Country', 'Phone', 'Email');
            $callback = function() use ($exportChapterList, $columns)
            {
                $file = fopen('php://output', 'w');
                fputcsv($file, $columns);
...
/**
    ...
    public function exportEOYStatus()
    {
        ...
        if(sizeof($activeChapterList) > 0)
        {
            ...
            $columns = array('Conference', 'Region', 'State', 'Name', 'Board Report Received', 'Board Report Activated', 'Financial Report Received', 'Financial Report Reviewed', 'Primary Coordinator', 'Assigned Reviewer');
            $callback = function() use ($exportChapterList, $columns)
            {
                $file = fopen('php://output', 'w');
                fputcsv($file, $columns);
 ...
 /**
    ...
    public function exportIntEOYStatus()
    {
        ...
        if(sizeof($activeChapterList) > 0)
        {
            ...
            $columns = array('Conference', 'Region', 'State', 'Name', 'Board Report Received', 'Board Report Activated', 'Financial Report Received', 'Financial Report Reviewed', 'Primary Coordinator');
            $callback = function() use ($exportChapterList, $columns)
            {
                $file = fopen('php://output', 'w');
                fputcsv($file, $columns);
 ...
 /**
    ...
    public function exportCoordinator($id)
    {
        ...
        if(sizeof($exportCoordinatorList) > 0){
            $columns = array('Id', 'Conference', 'Region', 'First Name', 'Last Name', 'Position', 'Sec Position', 'Email', 'Sec Email', 'Report Id', 'Address', 'City', 'State', 'Zip', 'Phone', 'Alt Phone', 'Birthday Month', 'Birthday Day', 'Coordinator Start', 'Last Promoted', 'Last UpdatedBy', 'Last UpdatedDate');
            $callback = function() use ($exportCoordinatorList, $columns)
            {
                ...
                foreach($exportCoordinatorList as $list) {
                    fputcsv($file, array($list->coordinator_id,
                                            $list->conference_id,
                                            $list->reg_name,
                                            $list->first_name,
                                            $list->last_name,
    ...
    public function exportIntCoordinator()
    {
        ...
        if(sizeof($exportCoordinatorList) > 0){
            $columns = array('Conference', 'Region', 'First Name', 'Last Name', 'Position', 'Sec Position', 'Email', 'Sec Email', 'Reports To', 'Address', 'City', 'State', 'Zip', 'Phone', 'Alt Phone', 'Birthday Month', 'Birthday Day', 'Coordinator Start', 'Last Promoted', 'Last UpdatedBy', 'Last UpdatedDate');
            $callback = function() use ($exportCoordinatorList, $columns)
            {
                $file = fopen('php://output', 'w');
                fputcsv($file, $columns);
                ...
                foreach($exportCoordinatorList as $list) {
                    fputcsv($file, array($list->conference_id,
                                            $list->reg_name,
                                            $list->first_name,
                                            $list->last_name,
                                            $list->position,
    ...
    public function exportIntRetCoordinator()
    {
        ...
        if(sizeof($exportCoordinatorList) > 0){
            $columns = array('Conference', 'Region', 'First Name', 'Last Name', 'Position', 'Sec Position', 'Email', 'Sec Email', 'Reports To', 'Address', 'City', 'State', 'Zip', 'Phone', 'Alt Phone', 'Birthday Month', 'Birthday Day', 'Coordinator Start', 'Last Promoted', 'Last UpdatedBy', 'Last UpdatedDate','Disband Date','Disband Reason');
            $callback = function() use ($exportCoordinatorList, $columns)
            {
                $file = fopen('php://output', 'w');
                fputcsv($file, $columns);
...
public function exportRetiredCoordinator(Request $request){
   ...
   $tasks = CoordinatorDetails::select("*")
        ...
        //var_dump($_temp);die;
        $columns = array('Conference', 'Region', 'First Name', 'Last Name', 'Position', 'Sec Position', 'Email', 'Sec Email', 'Report Id', 'Address', 'City', 'State', 'Zip', 'Phone', 'Alt Phone', 'Birthday Month', 'Birthday Day', 'Coordinator Start', 'Last Promoted', 'Last UpdatedBy', 'Last UpdatedDate', 'Retire Date', 'Reason');
        ...
        $callback = function() use ($tasks, $columns, $_temp)
        {
            ...
            foreach ($tasks as $task) {
                ...
                fputcsv($file, array($row['Conference'], $row['Region'], $row['First Name'], $row['Last Name'], $row['Position'], $row['Sec Position'], $row['Email'], $row['Sec Email'], $row['Report Id'], $row['Address'], $row['City'], $row['State'], $row['Zip'], $row['Phone'], $row['Alt Phone'], $row['Birthday Month'], $row['Birthday Day'], $row['Coordinator Start'], $row['Last Promoted'], $row['Last UpdatedBy'], $row['Last UpdatedDate'], $row['Retire Date'], $row['Reason']));
            }
    ...
    function coordinatorPosition($id)
    {
        if($id != '0' && $id != ''){
    ...
    function get_reporting_coordinator($id){
        if($id != '0' && $id != ''){
...
public function exportAppreciation(Request $request){
   ...
   $exportCoordinatorList = CoordinatorDetails::select("*")
        ...
        //var_dump($_temp);die;
        $columns = array('Conference', 'Region', 'First Name', 'Last Name', 'Start Date', 'Position', 'Secondary Position', '<1 Year', '1 Year', '2 Years', '3 Years', '4 Years', '5 Years', '6 Years', '7 Years', '8 Years', '9 Years', 'Necklace', 'Top Tier/Other');
        $callback = function() use ($exportCoordinatorList, $columns)
        {
            $file = fopen('php://output', 'w');
            fputcsv($file, $columns);
            ...
            foreach ($exportCoordinatorList as $list) {
                ...
                fputcsv($file, array($row['Conference'], $row['Region'], $row['First Name'], $row['Last Name'], $row['Start Date'], $row['Position'], $row['Secondary Position'], $row['< 1 Year'], $row['1 Year'], $row['2 Years'], $row['3 Years'], $row['4 Years'], $row['5 Years'], $row['6 Years'], $row['7 Years'], $row['8 Years'], $row['9 Years'], $row['Necklace'], $row['Top Tier/Other']));
            }
 ...
 /**
    ...
    function exportChapterCoordinator(){
		...
		fputcsv($output, array('Conference','Region','State','Name', 'CC','ACC', 'RC', 'ARC', 'SC', 'AC', "Big"));
    ...
    function exportChapterAwardList(){
       	// output headers so that the file is downloaded rather than displayed
		header('Content-Type: text/csv; charset=utf-8');
		$today = date("Y-m-d");
		...
		fputcsv($output, array('State', 'Chapter', 'Award', 'Approved'));
		$award_array = null;
	    //Get Coordinators Details
        $corDetails = User::find(Auth::user()->id)->CoordinatorDetails;
