   ...
   /**
    ...
    public function memberUpdate(Request $request, $id)
    {
        ...
        try{
           ...
           Mail::send('emails.chaptermemberupdate', $mailData,function($message) use ($to_email)
            {   
                $message->to($to_email, 'MOMS Club')->subject('Database Update Auto-Notification'); 
            });
           ...
           //List Admin Notification//
            ...
            Mail::send('emails.listadminchaptermemberupdate', $mailData,function($message2) use ($to_email2)
            {   
                $message2->to($to_email2, 'MOMS Club')->subject('Chapter Update Request'); 
            });}
    ...
    public function createBoardInfo(Request $request, $chapter_id){
        ...
        $websiteLinkStatus = $request->get('ch_linkstatus');

        if ($websiteLinkStatus === '2' || $websiteLinkStatus === '3') {
            $chapter->website_link_status = $websiteLinkStatus;
        }
    ...
    public function showFinancialReport($chapterId)
{
    try {
      ...
      //  }
        ...
        $data = array('financial_report_array'=>$financial_report_array,'loggedInName'=>$loggedInName,'submitted' => $submitted,'chapterDetails' => $chapterDetails);
   ...
   public function load_coordinators($chId,$chName,$chState,$chConf,$chPcid)
    {
        ...
        $corList = DB::table('coordinator_details as cd')
            ...
            ->get();
            $coordinator_array[$i] = array('id' =>$corList[0]->cid,
		...
		for($i=0; $i<$coordinator_count; $i++){
			...
			else if($coordinator_array[$i]['position']=="CC"){
